...
------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------
--COMANDO PARA LA ELIMINACION EN CASCADA DEL PROYECTO
------------------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------
--Crea el Schema utilizado en la base de datos bajo el nombre 'BIG_DATA'
------------------------------------------------------------------------------------------------------------------------
...
---Creacion de las tablas que no poseen Claves foráneas (FK)
...
---Finalizacion creacion de las tablas que no poseen Claves foráneas (FK)
...
CREATE TABLE BIG_DATA.Hotel (
	...
	recargaEstrella NUMERIC (18,0) DEFAULT 10
...
------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------
--MIGRACION DE LOS DATOS DE LA TABLA MAESTRA A LAS TABLAS PROPIAS Y CREACION DE DATOS EN TABLAS CUYOS DATOS NO PROVENGAN
--DE LA TABLA MAESTRA
------------------------------------------------------------------------------------------------------------------------


--Migracion de la tabla maestra a la Tabla Regimen
...
--Migracion de la tabla maestra a la Tabla Tipo Habitacion
...
--Adicion de Valores a la Tabla EstadoReserva
...
--Adicion de Valores a la Tabla PaisNacionalidad
...
--Adicion de Valores a la Tabla TipoDocumento
	INSERT INTO [BIG_DATA].TipoDocumento (tipoDocumentoDesc)
		VALUES ('DNI'),('Pasaporte'),('L.Civil'),('Licencia'),('Enrolamiento'),('CUIT'),('CUIL')

--Adicion de Valores a la Tabla Rol
	INSERT INTO [BIG_DATA].[Rol] (rolDesc)
		VALUES ('Administrador'),('Recepcionista'),('Guest')
	
--Adicion de Valores a la Tabla Funcion
...
--Adicion de Valores a la Tabla Usuario
INSERT INTO [BIG_DATA].[Usuario] (username,nombre,apellido,tipo_Documento,documento,mail,telefono,direccion,fecha_Nacimiento,estadoUsuario,user_intentos_fallidos,userpassword)
		VALUES ('admin','Nombre admin','Apellido admin', 1, 11111111,'admin@administrador.com',1511111111,'Calle admin',01/01/1900,1,0,HASHBYTES('SHA2_256', CONVERT(nvarchar(255), 'w23e')))

--Migracion de la tabla maestra a la Tabla Consumible
...
--Migracion de la tabla maestra a la Tabla Ciudad
...
--Migracion de la tabla maestra a la Tabla Hotel
...
--Migracion de la tabla maestra a la Tabla Cliente
...
--Migracion de la tabla maestra a la Tabla Habitacion
...
--Migracion de la tabla maestra a la Tabla Reserva 
...
--Migracion de la tabla maestra a la Tabla Estadia
...
--Migracion de la tabla maestra a la Tabla Factura	
...
--Migracion de la tabla maestra a la Tabla Item
...
--Adicion de Valores a la Tabla RegimenXHotel
...
--Adicion de Valores a la Tabla UsuarioXHotel
	INSERT INTO [BIG_DATA].[UsuarioXHotel]
		SELECT distinct idHotel,username
		FROM [BIG_DATA].[Usuario],[BIG_DATA].[Hotel]
		WHERE username='admin'


--Adicion de Valores a la Tabla FuncionXRol
...
--Adicion de Valores a la Tabla UsuarioXRol
	INSERT INTO [BIG_DATA].[UsuarioXRol] (username,idRol)
	SELECT u.username, r.idRol
	FROM [BIG_DATA].[Usuario] u,[BIG_DATA].[Rol] r
	WHERE (u.username='admin' and r.idRol=1)

--Adicion de Valores a la Tabla ConsumiblesXEstadia
...
--Cancela todas aquellas reservas en las cuales el cliente no se peresento
GO
CREATE PROCEDURE BIG_DATA.Cancelar_Reservas_Noshow
...
UPDATE BIG_DATA.Reserva
SET idEstadoReserva = 5
WHERE idReserva = 1 AND CONVERT(date, @fechaActual) > CONVERT(date,fecha_Reserva_Desde)

END


--Da la posibilidad de modificar los datos de la reserva
GO
CREATE PROCEDURE [BIG_DATA].Modificar_Reserva 
@NumeroReserva numeric(18, 0),
@FechaDesde datetime,
@FechaHasta datetime,
@idTipoDeHabitacion numeric(18,0),
@idHotel numeric (18,0),
@idRegimen numeric(18, 0),
...
begin
	...
	begin
		...
		select @idHabitacion=h.idHabitacion from [BIG_DATA].[Habitacion] h where h.idHotel=@idHotel and h.numeroHab=@numeroHabitacion

		declare @estadoModificado numeric (18,0)
		select @estadoModificado=e.idEstadoReserva from [BIG_DATA].[EstadoReserva] e where e.estadoReservaDesc='Reserva Modificada'

		UPDATE [BIG_DATA].[Reserva] SET cantidadNoches=( ABS(DATEDIFF(day,@FechaHasta,@FechaDesde))),fecha_Reserva_Desde=@FechaDesde,
			idTipoHabitacion=@idTipoDeHabitacion, idHotel=@idHotel,idRegimen=@idRegimen,idHabitacion=@idHabitacion,
			idEstadoReserva=@estadoModificado,ultimaModificacionPor=@username
...
--Informa los clientes con mayor cantidad de puntos.
GO
...
BEGIN
	...
	BEGIN
		...
		IF ((SELECT COUNT(*) FROM BIG_DATA.Reserva WHERE idReserva = @idReserva AND idEstadoReserva IN(1,2)) = 0)
		BEGIN
			RAISERROR('La reserva no esta activa',16,1)
			RETURN
		END

		IF ((SELECT fecha_Reserva_Desde FROM BIG_DATA.Reserva WHERE idReserva = @idReserva) = @fecha_ing)
		BEGIN
		...
		BEGIN
			IF ((SELECT fecha_Reserva_Desde FROM BIG_DATA.Reserva WHERE idReserva = @idReserva) < @fecha_ing)
			BEGIN
				UPDATE [BIG_DATA].[Reserva] SET idEstadoReserva=5 WHERE idReserva=@idReserva	
				RAISERROR('La reserva quedo sin validez',16,1)
				return
			END
			ELSE
			BEGIN
				RAISERROR ('No se puede realizar el check in antes de la fecha de inicio de la reserva',16,1)
			END
		END
	END
END
...
AS
BEGIN
	IF ((select fecha_Check_Out FROM BIG_DATA.Estadia where idReserva = @idReserva) IS NOT NULL)
	BEGIN
		RAISERROR('Ya realizo el check out para esta estadia',16,1)
		RETURN
	END

	if ((select e.fecha_Check_Out from [BIG_DATA].[Estadia] e where e.idReserva=@idReserva) IS NULL and 
		(select e.fecha_Check_In from [BIG_DATA].[Estadia] e where e.idReserva=@idReserva) IS NOT NULL)
	begin
		DECLARE @fechaIn datetime = (SELECT fecha_Check_In FROM BIG_DATA.Estadia WHERE idReserva = @idReserva)
		DECLARE @cantidadDias numeric (18,0) = ABS((CAST(DATEDIFF(day,@fecha_eg,@fechaIn) AS numeric(18,0))))
