...
BEGIN
	...
	BEGIN TRY
		...
		INSERT INTO [BIG_DATA].[UsuarioXRol] (username,idRol) VALUES (@Username,@idRol)

	END TRY
	
	BEGIN CATCH
			RAISERROR('El usuario ya se encuentra registrado',16,1) --POR SI TIRA ERROR DE USERNAME O DE MAIL Q TIENEN Q SER UNICOS
	END CATCH
END
GO
...
/*
go
CREATE PROCEDURE  BIG_DATA.isAllInclusive 
@idReserva numeric (18,0)
as 
begin

	declare @esAllIncluseve int = (Select COUNT(*) from [BIG_DATA].[Regimen] R, BIG_DATA.Reserva RE WHERE R.idRegimen = RE.idRegimen AND R.descripcion='All inclusive')

	if (@esAllIncluseve > 0)
	BEGIN
		declare @idEstadia numeric(18,0) = (select e.idEstadia from [BIG_DATA].[Estadia] e where e.idReserva= @idReserva)

		select (sum(CE.cantidad * C.consumiblePrecio)*-1) 
		  from [BIG_DATA].[ConsumibleXEstadia] ce inner join [BIG_DATA].[Consumible] c on c.idConsumible=ce.idConsumible 
		  where ce.idEstadia=@idEstadia group by ce.idEstadia
	end
end
GO
*/
...
begin
	...
	UNION 
	SELECT 'Descuento por regimen de estadia', SUM(c.consumiblePrecio*ce.cantidad) * -1 AS TOTAL, SUM(ce.cantidad) 
	FROM BIG_DATA.ConsumibleXEstadia ce inner join BIG_DATA.Consumible c on c.idConsumible=ce.idConsumible 
	WHERE ce.idEstadia=@idEstadia
	GROUP BY idEstadia
end
...
begin
	...
	SELECT CONVERT(NUMERIC(16,2),(R.precio * T.precio * H.cantidadDeEstrellas) * E.cantidadDias) AS TOTALESTADIA, e.cantidadDias,
				CONVERT(NUMERIC(16,2),(R.precio * T.precio * H.cantidadDeEstrellas) * (ABS(DATEDIFF(day,re.fecha_Reserva_Hasta,e.fecha_Check_Out)))) AS TOTALRESTANTE,
				ABS(DATEDIFF(day,re.fecha_Reserva_Hasta,e.fecha_Check_Out)) as diferencia
		FROM BIG_DATA.TipoHabitacion T, BIG_DATA.Regimen R, BIG_DATA.Hotel H, BIG_DATA.Reserva RE, BIG_DATA.Estadia E where
		e.idReserva=re.idReserva and re.idHotel=h.idHotel and r.idRegimen=re.idRegimen and re.idTipoHabitacion=t.idTipoHabitacion and e.idReserva=@idReserva
